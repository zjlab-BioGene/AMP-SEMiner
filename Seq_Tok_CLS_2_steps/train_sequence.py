...
from transformers import EsmModel, EsmPreTrainedModel, EsmForTokenClassification, EsmForSequenceClassification
...
class AMPSequenceClassification(EsmForSequenceClassification):
    ...
    def forward(
        ...
        prolabels: Optional[torch.LongTensor] = None,
        ...
        loss = None
        prolabels = prolabels
        if prolabels is not None:
            prolabels = prolabels.to(logits.device)

            if self.config.problem_type is None:
                if self.num_labels == 1:
                    self.config.problem_type = "regression"
                elif self.num_labels > 1 and (prolabels.dtype == torch.long or prolabels.dtype == torch.int):
            ...
            if self.config.problem_type == "regression":
                loss_fct = MSELoss()
                if self.num_labels == 1:
                    loss = loss_fct(logits.squeeze(), prolabels.squeeze())
                else:
                    loss = loss_fct(logits, prolabels)
            elif self.config.problem_type == "single_label_classification":
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits.view(-1, self.num_labels), prolabels.view(-1))
...
def get_parameters():
    ...
    parser.add_argument('--label_path', type=str, default='./example_labels')
...
def model_load(model_name,num_labels,ft_mode,lora_rank,focal,alpha,beta):
    ...
    if ft_mode == "freeze":
        model = AMPSequenceClassification.from_pretrained(model_name, num_labels=num_labels, freeze=True)
    elif ft_mode == 'lora':
        model = AMPSequenceClassification.from_pretrained(model_name, num_labels=num_labels, freeze=False)
...
def train_args_prepare(model_name, learning_rate, batch_size, num_train_epochs, weight_decay, ft_mode, lora_rank):
    # folder_name = f"{model_name}-ft-for-Sequence-and-Token-classification"
    mol_name = model_name.split('/')[-1]
    folder_name = f"finetune/{mol_name}-ft-for-sequence-classification-full-finetune"
    if ft_mode == 'lora':
